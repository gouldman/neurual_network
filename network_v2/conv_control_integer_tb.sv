...
logic [input_data_bits-1:0] parallel_previous_result; // Previous convolution result
logic parallel_previous_result_valid;          // Valid signal for previous result
...
logic [input_data_bits:0] parallel_conv_result; // Convolution result
logic [$clog2(pic_size*pic_size/serial_to_parallel_coe)-1:0] parallel_conv_result_addr; // Address for convolution result
logic parallel_conv_result_valid;              // Valid signal for convolution result
...
conv_control_integer dut (
    ...
    .parallel_previous_result(parallel_previous_result),
    .parallel_previous_result_valid(parallel_previous_result_valid),
    .parallel_weight_data(parallel_weight_data),
    .parallel_weight_data_valid(parallel_weight_data_valid),
    .need_pic(need_pic),
    .conv_finish(conv_finish),
    .parallel_conv_result(parallel_conv_result),
    .parallel_conv_result_addr(parallel_conv_result_addr),
    .parallel_conv_result_valid(parallel_conv_result_valid),
  ...
  task automatic reset_dut();
    begin
      ...
      conv_start = 0;
      parallel_previous_result_valid = '0;
      parallel_previous_result = '0;
